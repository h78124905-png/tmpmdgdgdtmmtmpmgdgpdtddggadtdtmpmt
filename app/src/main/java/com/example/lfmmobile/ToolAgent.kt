...
class ToolAgent(private val engine:LlamaEngine,private val search:SearchService){
    ...
    suspend fun run(initialMessages:JSONArray,maxTokens:Int):AgentResult=withContext(Dispatchers.Default){
        val messages=initialMessages;val tools=toolDefinitions();val sources=mutableListOf<SearchResult>();var thinking=""
        repeat(MAX_TOOL_CALLS+1){
            ...
            when(step.optString("type")){
                ...
                "tool_calls"->{
                    val calls=step.optJSONArray("calls")?:return@withContext AgentResult("",thinking,sources,"Tool call list missing");thinking+=step.optString("reasoning")
                    if(calls.length()==0||calls.length()>4)return@withContext AgentResult("",thinking,sources,"Invalid number of tool calls")
                    val assistantCalls=JSONArray()
                    for(i in 0 until calls.length()){
                        val c=calls.optJSONObject(i)?:return@withContext AgentResult("",thinking,sources,"Invalid tool call");val name=c.optString("name");val argsText=c.optString("arguments");val id=c.optString("id").ifBlank{"call_${i+1}"};val args=try{JSONObject(argsText)}catch(_:Exception){return@withContext AgentResult("",thinking,sources,"Invalid arguments for $name")}
                    ...
                    for(i in 0 until calls.length()){
                        val c=calls.getJSONObject(i);val name=c.getString("name");val args=JSONObject(c.getString("arguments"));val id=c.optString("id").ifBlank{"call_${i+1}"};val result=withContext(Dispatchers.IO){execute(name,args)};sources+=result.sources
