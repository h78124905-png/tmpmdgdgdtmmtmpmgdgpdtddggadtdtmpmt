...
root = Path('.')
native = root / 'app/src/main/cpp/native-lib.cpp'
tool = root / 'app/src/main/cpp/tool-calling.cpp'

s = native.read_text(encoding='utf-8')
if '#include <functional>' not in s:
    s = s.replace('#include <exception>\n', '#include <exception>\n#include <functional>\n', 1)

sig = 'std::string generate_impl(JNIEnv * env, const std::string & prompt_text, int max_tokens, jobject callback) {'
start = s.index(sig)
brace = s.index('{', start)
depth = 0
end = None
for i in range(brace, len(s)):
    if s[i] == '{': depth += 1
        ...
        if depth == 0:
            ...
            break
if end is None:
    raise SystemExit('could not find generate_impl end')
old = s[start:end]
marker = '    jmethodID on_token = nullptr; jmethodID on_stats = nullptr;'
marker_pos = old.index(marker)
core = old[marker_pos:]
core = core.replace('    // Baseline conversation path: always start from a clean KV state.', '    if (progress) progress("kv_clear");\n\n    // Shared generation path: always start from a clean KV state.', 1)
core = core.replace('    const auto prefill_start = std::chrono::steady_clock::now();', '    if (progress) progress("prefill");\n    const auto prefill_start = std::chrono::steady_clock::now();', 1)
core = core.replace('    LOGI("[prefill] completed prompt_tokens=%zu elapsed_ms=%lld speed=%.2f tok/s", input.size(), static_cast<long long>(prefill_ms), prefill_ms > 0 ? static_cast<double>(to_eval) * 1000.0 / prefill_ms : 0.0);', '    LOGI("[prefill] completed prompt_tokens=%zu elapsed_ms=%lld speed=%.2f tok/s", input.size(), static_cast<long long>(prefill_ms), prefill_ms > 0 ? static_cast<double>(to_eval) * 1000.0 / prefill_ms : 0.0);\n    if (progress) progress("prefill_complete");', 1)
core = core.replace('    std::string output; std::string utf8_pending;', '    if (progress) progress("generation");\n    std::string output; std::string utf8_pending;', 1)

helper_prefix = '''std::string generate_chat_impl(JNIEnv * env, const common_chat_params & chat_params, const llama_tokens & input, int max_tokens, jobject callback, const std::function<void(const char *)> & progress) {
...
'''
helper = helper_prefix + core
wrapper = '''std::string generate_impl(JNIEnv * env, const std::string & prompt_text, int max_tokens, jobject callback) {
    if (!g_engine.model || !g_engine.context) return "[model not loaded]";
...
ts = 'extern "C" JNIEXPORT jstring JNICALL\nJava_com_example_lfmmobile_LlamaEngine_nativeGenerateToolStep'
tstart = t.index(ts)
tbrace = t.index('{', tstart)
tdepth = 0
tend = None
for i in range(tbrace, len(t)):
    if t[i] == '{': tdepth += 1
    elif t[i] == '}':
        tdepth -= 1
        if tdepth == 0:
            ...
            break
if tend is None:
    raise SystemExit('could not find nativeGenerateToolStep end')

new_tool = r'''extern "C" JNIEXPORT jstring JNICALL
...
t = t[:tstart] + new_tool + t[tend:]
tool.write_text(t, encoding='utf-8')

ns = native.read_text(encoding='utf-8')
ts2 = tool.read_text(encoding='utf-8')
assert ns.count('std::string generate_chat_impl(') == 1
assert ns.count('std::string generate_impl(') == 1
assert 'sampling.grammar' not in ts2
assert 'common_sampler_init(g_engine.model, sampling)' not in ts2
assert 'llama_memory_clear(llama_get_memory(g_engine.context)' not in ts2
