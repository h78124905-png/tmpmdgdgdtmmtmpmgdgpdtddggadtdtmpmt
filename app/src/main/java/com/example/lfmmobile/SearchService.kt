...
class SearchService(private val bridgeUrl: String = DEFAULT_BRIDGE_URL, private val bearerToken: String = "") {
    ...
    fun searchAndFetch(query: String, searchResults: Int = 8, fetchResults: Int = 3): String? {
        val args = JSONObject().put("query", query).put("search_results", searchResults.coerceIn(1, 20)).put("fetch_results", fetchResults.coerceIn(1, 5)).put("max_characters_per_page", 20000).put("total_character_budget", 60000).put("timeout_ms", 45000)
        return callBridgeText("search_and_fetch", args)
    }

    fun fetchUrl(url: String): String? {
        val args = JSONObject().put("url", url).put("output", "markdown").put("max_characters", 20000).put("include_metadata", true).put("include_links", false).put("timeout_ms", 15000)
    ...
    private fun parseResults(array: JSONArray?, limit: Int): List<SearchResult> = buildList {
        ...
        for (i in 0 until minOf(array.length(), limit)) {
            ...
            if (url.isNotBlank()) add(SearchResult(item.optString("title").ifBlank { "Web result" }, url, item.optString("snippet").ifBlank { item.optString("description") }))
        }
    }
